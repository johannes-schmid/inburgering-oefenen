...
const EXERCISE_SET: ReadonlySet<string> = new Set(EXERCISE_KINDS);
...
export function tierBucket(tier: Tier): 'receptief' | 'productief' {
 ...
 */
const safeHtml = nonEmpty.refine(
...
const withOneGap = nonEmpty.refine(
  s => s.split(GAP).length === 2,
  { message: `de zin moet precies één gat bevatten, geschreven als ${GAP}` },
);
...
export const woordordePayload = z.object({
  ...
  v => v.tokens.length === v.answer.length
    && [...v.tokens].sort().join(' ') === [...v.answer].sort().join(' '),
  { message: 'answer moet dezelfde tokens bevatten als tokens, in de juiste volgorde', path: ['answer'] },
);
...
/** Eén item met zijn payload al geparseerd naar de vorm die bij zijn `kind` hoort. */
...
export type ItemIssue = { index: number; kind: string; message: string };
 ...
 */
export function validateItem(raw: unknown, index: number): ItemIssue[] {
  ...
  const push = (message: string) => issues.push({ index, kind: item.kind, message });
  ...
  if (isExercise && item.tier === null) push('een opgave moet een tier hebben (0, 1 of 2)');
  if (!isExercise && item.tier !== null) push('alleen een opgave heeft een tier');
  ...
  if (isExercise && !item.explanation?.trim()) push('een opgave moet een explanation hebben');
  ...
  if (item.kind === 'mcq') {
    ...
    if (correct !== 1) push(`mcq heeft precies één juist antwoord, niet ${correct}`);
    const labels = item.options.map(o => o.label);
    if (new Set(labels).size !== labels.length) push('mcq heeft een dubbel optielabel');
 ...
 */
export function validateItems(items: unknown[]): ItemIssue[] {
...
export function matchesTyped(given: string, answer: string, accept: string[] = []): boolean {
