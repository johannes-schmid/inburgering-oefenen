...
export function day2Email(
  ...
  const examName = payload.exam_name ?? 'KNM Proefexamen';
  const isGood = pct >= 60;
  const score = payload.score ?? 0;
  const total = payload.total ?? 44;

  const dashboardUrl = 'https://inburgeringoefenen.nl/dashboard';

  // Personalised threshold insight
  const needed = Math.max(0, 27 - score);
  const above = Math.max(0, score - 27);
  let insightBadge: string;
  if (pct >= 80) insightBadge = locale === 'ar' ? `${above} سؤالاً فوق الحد` : locale === 'en' ? `${above} questions above the threshold` : `${above} vragen boven de slagingsgrens`;
  else if (isGood) insightBadge = locale === 'ar' ? `${above} سؤالاً فوق الحد` : locale === 'en' ? `${above} ${above === 1 ? 'question' : 'questions'} above the pass mark` : `${above} ${above === 1 ? 'vraag' : 'vragen'} boven de slagingsgrens`;
  else insightBadge = locale === 'ar' ? `${needed} سؤالاً بعد` : locale === 'en' ? `${needed} more ${needed === 1 ? 'question' : 'questions'} to go` : `Nog maar ${needed} ${needed === 1 ? 'vraag' : 'vragen'} te gaan`;
...
<div style="background:linear-gradient(160deg,#001040 0%,#002b6d 45%,#0f4299 100%);padding:36px 40px 40px;text-align:center;position:relative;overflow:hidden;">
  ...
  <div style="display:inline-block;background:${isGood ? 'rgba(22,163,74,0.25)' : 'rgba(220,38,38,0.25)'};border:1px solid ${isGood ? 'rgba(22,163,74,0.5)' : 'rgba(220,38,38,0.5)'};color:${isGood ? '#6ee7b7' : '#fca5a5'};font-size:12px;font-weight:700;padding:5px 16px;border-radius:99px;margin-bottom:18px;">${isGood ? (locale === 'ar' ? '✓ نجحت' : locale === 'en' ? '✓ Passed' : '✓ Geslaagd') : (locale === 'ar' ? '✗ لم تنجح بعد' : locale === 'en' ? '✗ Not yet passed' : '✗ Nog niet geslaagd')}</div>
...
<div class="sec" style="background:#f8f9fb;">
  ...
  <p class="sec-title" style="text-align:center;">${sc.incl}</p>
  ${packageCards(locale)}
