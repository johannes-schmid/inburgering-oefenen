...
**Outcome:** `SUCCESS` (tsc clean, `next build` clean met 227 static pages, 575/575 vitest groen; alles nagemeten tegen `next start` op 3099)

**What went wrong:** Twee dingen. Ten eerste dacht ik dat `app/[locale]/opengraph-image.tsx` als bestandsconventie genoeg was. Dat is het niet: Next vult daarmee alleen een pagina aan die zélf geen `openGraph` opgeeft. Gemeten op de draaiende build had `/nl/premium` (geen eigen blok) wél een `og:image` en hadden `/nl`, `/nl/gidsen` en `/nl/blog` (wel een eigen blok) er géén. Pas met `images: ogImageFor(locale)` in elk van die blokken staat hij overal. Ten tweede: twee bevindingen uit de audit waren fout en zijn níet "gerepareerd" — de site zou zogenaamd nul hreflang-tags hebben (een grep op `hreflang="` mist wat Next als `hrefLang` schrijft; ze staan er alle vier, inclusief `x-default`), en vier Arabische blogposts zouden uit de sitemap ontbreken (ze staan op `noindex, follow`, en `app/sitemap.ts` laat ze er precies daarom uit — het commentaar bij `hasTranslation` zegt dat al).

**Lesson:** Een metadata-bestandsconventie is een aanvulling op een pagina zonder eigen `openGraph`, geen overschrijving van een pagina mét. Controleer zo'n aanname tegen een echte `next start` en niet tegen de documentatie of de build-output — het verschil tussen `/premium` en `/gidsen` was op geen andere manier zichtbaar. En bij een bevinding van buitenaf: meet hem eerst zelf na tegen de bron. Een `noindex`-pagina die uit de sitemap is gelaten is geen gat maar een besluit, en dat besluit stond hier al opgeschreven.
...
## 2026-09-21 — De uitgang van de taster wijst het onderdeel aan, en de gastgrens is een overlay geworden
**Changed:** `FreePracticeEngine` linkt naar `/dashboard/<level>/<onderdeel>` (of `/dashboard/knm`) in plaats van `/dashboard`; de twee spelerpagina's onder `(app)/oefenexamen/` laten een gast toe op een **gratis** examen en geven `ExamShell` een `guest`-prop; nieuw `components/exam/GuestSignupOverlay.tsx`; `GUEST_PREVIEW_QUESTIONS = 5` in `lib/features.ts`; `exam-slots.ts` opent het gratis slot voor een gast; nieuwe flow `scripts/walkthrough/flows/gast-naar-account.mjs`.
**Outcome:** SUCCESS
...
## 2026-09-22 — MRR en churn op het admin-dashboard
**Changed:** `lib/admin/mrr.ts` (pure `summariseSubscriptions`), `app/[locale]/(admin)/admin/_components/MrrCard.tsx`, aangehaakt in `app/[locale]/(admin)/admin/page.tsx`, test in `tests-unit/mrr.test.ts`.
**Outcome:** SUCCESS
**What worked / went wrong:** Er is geen abonnementen-tabel — het abonnement staat in `auth.users.user_metadata` (`modules`, `subscription_canceled_at`, `modules_until`). De userlijst die het dashboard al paginaseerde voor de conversiegrafiek draagt nu ook de MRR, dus er komt geen tweede `listUsers`-ronde bij. Prijs per klant via `priceForSelection`, niet modules × €9,95, anders mist de bundelkorting.
**Lesson:** MRR is niet af te leiden uit `payments` — dat is gefactureerde omzet en telt de lopende maand niet. Een opgezegd abonnement telt niet als MRR, ook niet zolang de betaalde periode loopt; het staat apart als "opgezegd, loopt nog" zodat zichtbaar is wat er van de MRR af gaat. Legacy `plan`-accounts blijven buiten de MRR: eenmalige betaling, geen incasso.

## 2026-09-22 — MRR-bewegingsgrafiek (waterval) op het admin-dashboard
**Changed:** `buildMrrMovements()` in `lib/admin/mrr.ts`, `app/[locale]/(admin)/admin/_components/MrrMovementChart.tsx` (shadcn `Card` + `ChartContainer`, gestapelde `ComposedChart` met `stackOffset="sign"`), aangehaakt in `admin/page.tsx`, drie tests bij in `tests-unit/mrr.test.ts`.
**Outcome:** SUCCESS
**What worked / went wrong:** De instroom komt uit `payments` — en dat werkt juist omdát alleen `/api/checkout-modules` een rij schrijft: de tabel bevat eerste betalingen en bijkopen, geen maandelijkse incasso's, en dat is precies wat een bewegingsgrafiek nodig heeft. De uitstroom komt uit `subscription_canceled_at` + `modules_until`. De MRR-lijn wordt teruggerekend vanaf de MRR van de tegel, zodat de grafiek nooit een ander bedrag kan beweren dan het getal erboven. Twee dingen die misgingen: `ChartLegend` van recharts 3 accepteert geen `payload` meer (volgorde stond omgekeerd — nu een eigen legenda), en `check-ui-auth.mjs` fotografeert **elke** recharts-grafiek leeg (Chromium 101 in de harness) — geverifieerd met een los Playwright-script tegen de geïnstalleerde Chrome (`channel: 'chrome'`).
**Lesson:** Een recharts-grafiek valt niet te controleren met `check-ui.mjs`/`check-ui-auth.mjs`; de shot is blanco en dat zegt niets over de pagina. Gebruik daarvoor een echte Chrome. En: elke admin-query die een hele tabel optelt gaat door `fetchAll` — de betalingshistorie hier moet compleet zijn, want een ontbrekende oudste betaling laat een bestaande klant opnieuw als "nieuw" tellen.

## 2026-09-25 — SEO: de zoekwoorden op de geldpagina's, en een niveau-overzicht per niveau
**Changed:** (1) `messages/{nl,en,ar}.json`: homepage-titel en H1 weer zoekwoord-eerst ("Inburgeringsexamen oefenen — A2, B1 en KNM"), `/oefenen` op "oefenen voor inburgering", `oefenexamen.meta_title`/`heading` op "Inburgeringsexamen {skill} {level} oefenen" (EN: "Dutch {level} {skill} exam practice"), KNM op "KNM oefenen voor je inburgering"; nieuwe namespaces `oefenexamen.level.*` en `oefenexamen.how.<onderdeel>.*`. (2) Nieuwe route `app/[locale]/(main)/oefenexamen/[level]/page.tsx` (+ `i18n/routing.ts`, `app/sitemap.ts`, `scripts/check-schema.mjs`, `tests/seo.spec.js`): CollectionPage + ItemList naar de Course-nodes + FAQPage, geen eigen Course. (3) `oefenexamen/[level]/[skill]/page.tsx`: 300–400 woorden uitleg uit de messages, FAQ, niveau-kruimel, link omhoog. (4) Vier gidsen hertiteld op de Semrush-termen (`taalexamens-a2-b1` → "Het inburgeringsexamen: …", `welke-wet-en-welke-route` → "Wet inburgering 2021 of 2013: …", `pvt-map-en-ona` → "MAP, PVT en ONA …", `moet-ik-inburgeren` → "… Wie is inburgeringsplichtig"), NL + EN. (5) `app/sitemap.ts`: de drie noindex-juridische pagina's eruit, `lastModified` alleen nog waar hij echt is (blog, gidsen). (6) `encodeURI` om drie `redirect()`-aanroepen met een mogelijk Arabisch pad. (7) `components/Footer.tsx` en de hero-chips linken naar de niveau-overzichten. (8) `SEO/semrush/keywords-nl-2026-09-25.csv` (nieuw), `SEO/keywords.md` (echte volumes bovenaan, de `[EST]`-tabellen als historie), `SEO/used-keywords.md`. (9) In `knm-website`: `components/Footer.tsx` + `messages/*.json` linken naar inburgeringoefenen.nl (niet gecommit).
**Outcome:** SUCCESS — tsc clean in beide repo's, `next build` clean (de `MISSING_MESSAGE gidsen.filter_*` in de log is van het niet-gecommitte `GuideIndex`-werk, niet van deze sessie), 614/614 vitest, nieuwe pagina's gemeten op de dev-server in drie talen.
**What worked / went wrong:**
- **De vergelijking met knmoefenen.nl was de diagnose, en de uitkomst was niet techniek.** KNM rankt mét dynamische rendering, kapotte canonicals en 404's in de sitemap; deze site is technisch beter en rankte niet. Het verschil: leeftijd (6 vs 2 maanden), KD (14–15 vs 23–40) bij nul backlinks aan beide kanten, en het woord "inburgering" dat op geen enkele oefenpagina in titel of H1 stond — GSC had nul vertoningen op elke query met "inburgering" erin. De H1 was op 07-28 bewust zoekwoord-eerst gezet en op 21-09 bij de hero-verbouwing stil vervangen door "Alle examenonderdelen in één platform".
- **Het niveau had geen URL.** Semrush zet ~3.500 zoekopdrachten per maand op "het A2-examen als geheel" (`inburgering examen oefenen a2` 880/23, `oefenen examen a2` 1.300/18, `inburgering examen a2` 1.300/34); de site sprong van de homepage naar de vier onderdeelpagina's. `/oefenexamen/a2` en `/b1` zijn die pagina, in de vorm van knmoefenen.nl's landingspagina's.
- **`title.template` in de layout is bewust níét toegevoegd**, hoewel het plan het noemde: elke titel krijgt er dan 22 tekens bij en de projectregel is ≤60. De suffixen blijven per pagina.
- **`tHow.has('p_a2_only')`** is hoe een onderdeel zonder A2-specifieke alinea (Lezen, Luisteren) dezelfde template deelt met Schrijven en Spreken, zonder lege sleutels in de andere talen.
- Twee scriptfouten onderweg: een `assert` op de sitemap faalde ná een eerdere `write` in hetzelfde script (het bestand stond half bewerkt), en EN `how_title` gebruikt `{skill_lower}` waar NL `{skill}` gebruikt — een sleutel die in drie talen andere variabelen vraagt, moet je in alle drie lezen vóór je hem aanroept.
**Lesson:** Zet het zoekwoord op de pagina die het moet winnen, niet op de pagina die erover praat. En als de zoekterm tussen twee bestaande niveaus van de site in valt (site → onderdeel), ontbreekt er een pagina, geen titel.
