...
export default function LerenThemaClient({ locale, slug }: { locale: string; slug: string }) {
  ...
  /**
   * The highest section index this user has ever opened, as stored.
   *
   * `user_leren_progress.max_section` is a high-water mark, so it may never go down —
   * paging back to section 1 must not throw away that you once reached section 7. There is
   * no `GREATEST()` through PostgREST's upsert, so the comparison has to happen here, which
   * means the stored value has to be read once. `0` until that read lands: writing a lower
   * number is the failure to avoid, and the guard below only writes on a strict increase.
   */
  const maxSectionRef = useRef(0);
  ...
  useEffect(() => {
    if (!userId || !thema) return;
    supabase
      .from('user_leren_progress')
      .select('max_section')
      .eq('user_id', userId)
      .eq('thema_id', thema.id)
      .maybeSingle()
      .then(({ data }) => { maxSectionRef.current = data?.max_section ?? 0; });
  }, [userId, thema?.id]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function nextQuestion() {
    ...
    if (next >= quizState.questions.length) {
      ...
      if (userId && thema) {
        ...
        }, { onConflict: 'user_id,thema_id' }).then(({ error }) => {
          // Never swallowed. `max_section` and `completed_at` did not exist on this table
          // until 20260828000000, so both upserts here answered 400 for as long as the page
          // has existed — and with `.then(() => {})` nothing said so. No KNM lesson progress
          // was ever recorded. A dropped write is invisible unless someone logs it.
          if (error) console.error('[leren] failed to record thema completion', error.message);
        });
  ...
  function goToSection(idx: number) {
    ...
    if (userId && thema && idx + 1 > maxSectionRef.current) {
      maxSectionRef.current = idx + 1;
      supabase.from('user_leren_progress').upsert({
        user_id: userId, thema_id: thema.id, max_section: idx + 1,
      }, { onConflict: 'user_id,thema_id' }).then(({ error }) => {
        if (error) console.error('[leren] failed to record section progress', error.message);
      });
    }
  }
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        #section-nav-bar { position:fixed; bottom:0; left:var(--portal-chrome-w,264px); right:0; z-index:50; background:#f8f9fb; border-top:1px solid #e6e8ea; padding:12px 24px; }
