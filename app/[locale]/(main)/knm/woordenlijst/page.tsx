/**
 * Free KNM vocabulary. Note this route is a static child of `/knm/[thema]` and wins over it.
 *
...
const HREF = '/knm/woordenlijst' as const;
const PATH = 'knm/woordenlijst';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'planned.knm_woorden' });
  return {
    title: t('meta_title'),
...
export default async function KnmWoordenlijstPage({ params }: Props) {
  const { locale } = await params;
