/**
 * Free A2/B1 exam vocabulary. Static child of `/taalexamens/[slug]`, and wins over it.
 *
...
const HREF = '/taalexamens/woordenlijst' as const;
const PATH = 'taalexamens/woordenlijst';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'planned.taal_woorden' });
  return {
    title: t('meta_title'),
...
export default async function TaalWoordenlijstPage({ params }: Props) {
  const { locale } = await params;
